...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.categories.update", [$category->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                <label for="customer_id">{{ trans('cruds.category.fields.customer') }}</label>
                <select class="form-control select2 {{ $errors->has('customer') ? 'is-invalid' : '' }}" name="customer_id" id="customer_id">
                    @foreach($customers as $id => $customer)
                        <option value="{{ $id }}" {{ (old('customer_id') ? old('customer_id') : $category->customer->id ?? '') == $id ? 'selected' : '' }}>{{ $customer }}</option>
                    @endforeach
                </select>
                ...
                @endif
                <span class="help-block">{{ trans('cruds.category.fields.customer_helper') }}</span>
            </div>
            <div class="form-group">
...
@if(isset($category) && $category->image)
      var files = {!! json_encode($category->image) !!}
          for (var i in files) {
          var file = files[i]
