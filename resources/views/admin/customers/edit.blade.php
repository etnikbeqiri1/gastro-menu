...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.customers.update", [$customer->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                <label for="menu_domain">{{ trans('cruds.customer.fields.menu_domain') }}</label>
                <input class="form-control {{ $errors->has('menu_domain') ? 'is-invalid' : '' }}" type="text" name="menu_domain" id="menu_domain" value="{{ old('menu_domain', $customer->menu_domain) }}">
                @if($errors->has('menu_domain'))
                    <div class="invalid-feedback">
                        {{ $errors->first('menu_domain') }}
                    </div>
                @endif
                <span class="help-block">{{ trans('cruds.customer.fields.menu_domain_helper') }}</span>
            </div>
            <div class="form-group">
                <label for="user_id">{{ trans('cruds.customer.fields.user') }}</label>
                <select class="form-control select2 {{ $errors->has('user') ? 'is-invalid' : '' }}" name="user_id" id="user_id">
                    @foreach($users as $id => $user)
                        <option value="{{ $id }}" {{ (old('user_id') ? old('user_id') : $customer->user->id ?? '') == $id ? 'selected' : '' }}>{{ $user }}</option>
                    @endforeach
                </select>
                @if($errors->has('user'))
                    <div class="invalid-feedback">
